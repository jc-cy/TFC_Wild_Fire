...
$targets = @(
  ...
  @{ Dir = 'mods'; Pattern = 'obscure_tooltips-forge-*.jar' },
  @{ Dir = 'mods'; Pattern = 'obscure_tooltips_fix-*.jar' },
  @{ Dir = 'mods'; Pattern = 'immersiveoverlays-*.jar' },
  @{ Dir = 'mods'; Pattern = 'Drip Sounds-*.jar' },
  @{ Dir = 'mods'; Pattern = 'particular-*.jar' },
  @{ Dir = 'mods'; Pattern = 'particle_core-*.jar' },
  @{ Dir = 'mods'; Pattern = 'bc_particle-*.jar' },
...
foreach ($target in $targets) {
  ...
  if ($disabled.Count -eq 0) {
    ...
    if ($active.Count -gt 0) {
      foreach ($file in $active) {
        Write-Host "[SKIP] Already in client mode: $($file.Name)"
        $skipped++
      }
    } else {
  ...
  foreach ($file in $disabled) {
    $newName = $file.Name.Substring(0, $file.Name.Length - $suffix.Length)
    ...
    Write-Host "[OFF] $($file.Name) -> $newName"
    $renamed++
  }
}

Write-Host ""
Write-Host "Server mode disabled. Renamed: $renamed, skipped: $skipped, missing: $missing"
